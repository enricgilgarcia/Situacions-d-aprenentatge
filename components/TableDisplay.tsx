...
export const TableDisplay: React.FC<TableDisplayProps> = ({ data, onEdit }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      alert("No s'ha pogut generar el PDF. Prova amb 'Imprimir' i triar 'Guardar com a PDF'.");
    } finally {
    ...
    const doc = new Document({
      sections: [{
        ...
        children: [
          ...
          // Seccions següents...
          new Paragraph({ text: "DESCRIPCIÓ (Context + Repte)", bold: true, size: 24, font: "Arial", spacing: { before: 400, after: 200 } }),
          new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: [new TableRow({ children: [createCell(data.descripcio.context_repte)] })] }),
          new Paragraph({ text: "COMPETÈNCIES ESPECÍFIQUES", bold: true, size: 24, font: "Arial", spacing: { before: 400, after: 200 } }),
          new Table({
            width: { size: 100, type: WidthType.PERCENTAGE },
            rows: [
              new TableRow({ children: [createCell("Competència", { bold: true, width: 70, isHeader: true }), createCell("Àrea", { bold: true, width: 30, isHeader: true })] }),
          ...
          new Paragraph({ children: [new PageBreak()] }),
          // Més taules simplificades per espai
          new Paragraph({ text: "OBJECTIUS I CRITERIS", bold: true, size: 24, font: "Arial", spacing: { before: 400, after: 200 } }),
          new Table({
            width: { size: 100, type: WidthType.PERCENTAGE },
            rows: [
              new TableRow({ children: [createCell("OBJECTIUS", { bold: true, width: 50 }), createCell("CRITERIS D'AVALUACIÓ", { bold: true, width: 50 })] }),
    ...
    <div className="bg-slate-200 p-0 md:p-8 print:p-0 min-h-screen">
      <style>{`
        ...
        .official-page { 
          background: white; 
          width: 297mm; 
          height: 209.5mm; /* Alçada calculada per encaixar en A4 horitzontal sense residus */
          padding: 15mm; 
          margin: 0;
        ...
        /* Evita la pàgina en blanc al final del PDF */
        ...
        .label-cell { width: 25%; font-weight: bold; background: #f9fafb; text-align: left; }
        .official-header { display: flex; align-items: center; margin-bottom: 30px; }
        .official-logo { height: 75px; }
        ...
        .footnote { position: absolute; bottom: 15mm; left: 15mm; right: 15mm; font-size: 9px; border-top: 1px solid #ddd; padding-top: 8px; color: #666; }
      ...
      <div ref={pdfRef} className="official-container">
        ...
        <div className="official-page">
          <div className="official-header">
             <img src={LOGO_GEN_BASE64} alt="Logo" className="official-logo" />
          </div>
          <div className="official-title-big text-slate-900">Situació d’aprenentatge<sup className="text-xl font-normal">1</sup></div>
          <div className="mt-28">
          ...
          <div className="page-num text-slate-400 uppercase tracking-tighter">Pàgina 1/5</div>
        </div>
        ...
        <div className="official-page">
          ...
          <table className="official-table">
            <thead><tr className="bg-slate-100 font-bold text-center"><td className="w-1/4">Fase</td><td>Activitat</td><td className="w-1/6">Hores</td></tr></thead>
            <tbody>
        ...
        <div className="official-page">
          ...
          <div className="section-title">SUPORTS UNIVERSALS (DUA)</div>
          <div className="box-content h-24">{data.vectors_suports.suports_universals}</div>
          <div className="section-title">SUPORTS ADDICIONALS O INTENSIUS</div>
          <table className="official-table">
            <thead><tr className="bg-slate-50 font-bold text-center"><td className="w-1/3">Alumne/a</td><td>Mesura / Suport</td></tr></thead>
            <tbody>
